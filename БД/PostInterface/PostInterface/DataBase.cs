...
namespace PostInterface
{
    class DataBase
    {
        ...
        /*
        public static void EditPostOffice(Post_Office Edited)
        {
            DeletePostOffice(Edited.Post_Index);
            AddPostOffice(Edited);
            //Context.SaveChanges();
        }
        */
        ...
        public static List<TrackElemClass> GetTrackListByConsigmentId(long ID_Consignment)
        {
            ...
            foreach (Track_List t in realTrackList)
            {
                ...
                tmp.DateTime = t.Date;
                Full_Letter_Status FLS = GetFullLetterStatusById(t.ID_Full_Letter_Status);
                tmp.FullStatus = FLS.Full_Status;
                tmp.Status = GetLetterStatusById(FLS.ID_Letter_Status).Status;
                tmp.PostIndex = t.Post_Index;
                tmp.ID_Letter = t.ID_Consignment;
                tmp.Сity = GetCityById(GetPostOfficeByPostIndex(t.Post_Index).ID_City).Name;
                res.Add(tmp);
            }
            return res;
        }
